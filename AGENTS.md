...
See intent files to understand what bgx is.
...
There are no backwards-compatibility concerns: this brand-new cli tool is in active
development and has never been deployed, so feel free to make sweeping changes
where necessary to better match the user's intent.

## Development

Rigorously test with a red-green TDD approach. E2E blackbox testing is
strongly preferred in general.
